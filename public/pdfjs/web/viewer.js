  ...
  load(pdfDocument) {
    ...
    firstPagePromise.then(pdfPage => {
      ...
      Promise.all([_ui_utils.animationStarted, storedPromise, pageLayoutPromise, pageModePromise, openActionPromise]).then(async ([timeStamp, stored, pageLayout, pageMode, openAction]) => {
        ...
        // zouhuili
        console.log(stored.scrollTop)
        const urlParams = _ui_utils.parseQueryString(location.search)
        if(urlParams.get('page')||urlParams.get('top')) {
          hash = `page=${urlParams.get('page')}&zoom=${zoom || stored.zoom},` + `${stored.scrollLeft},${urlParams.get('top')}`;
        }
